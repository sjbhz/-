...
function randomBlock(){
    ...
    for(var i in arr){
        if(arr[i]==0){
            ...
            //console.log(emptyPosition); //空的那个小格子随机定为一个0-8的数字
        }
...
function up(){
    if(emptyPosition<6){
        var targetIndex = parseInt(emptyPosition)+3;//4-9可向上移
        $("#main>div[data-index='"+targetIndex+"']").animate({
            top:'-=100px'
        },100,function(){
            $(this).attr('data-index',targetIndex-3)//向上一格数字相差3
        });
...
function down(){
    if(emptyPosition>2){
        var targetIndex=parseInt(emptyPosition)-3;//0-6可向下移
        $("#main>div[data-index='"+targetIndex+"']").animate({
            top:'+=100px'
        },100,function(){
